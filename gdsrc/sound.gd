...
func play_landing_sound(player_name: String,unit_id: String, pos: Vector3):
	set_stream(sound_map[LANDING])
	play()

func play_game_phase_start_sound()->void:
	set_stream(sound_map[PHASE_START])
	play()
	
func play_dice_sound()->void:
	set_stream(sound_map[DICE])
	play()
